...
\dirtree{%
.1 lib ( lokalne biblioteki).
.1 src.
.2 main.
.3 java ( kod źródłowy aplikacji ).
.3 resources ( pliki konfiguracyjne aplikacji ).
.3 webapp ( pliki dotyczące konfiguracji aplikacji internetowej ).
.2 test.
.3 java ( testy jednostkowe ).
.1 target ( zbudowane artefakty aplikacji ).
.1 pom.xml.
}
